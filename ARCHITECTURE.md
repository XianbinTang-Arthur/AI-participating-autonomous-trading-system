...
四主进程使用 NATS/hybrid 时，现行候选的 readiness ownership 只允许 Redis，键固定为全局 `aats:runtime:owner:<role>`。generation 仅存在 protocol v2 payload 和 peer barrier，不进入 key，避免旧/新 generation 通过不同 key 同时持有同一 role。在任何 NATS I/O 前，进程以 `SET NX PX` claim `PROVISIONING` payload，立即启动续租 task 和独立 subprocess watchdog，并完成 55 秒 takeover quarantine；隔离覆盖旧 v2 owner 从最后一次成功写 lease 到进程级 hard fence 的保守存活窗。父进程与 watchdog 统一使用计入宿主休眠的时钟（POSIX `CLOCK_BOOTTIME` / Windows `GetTickCount64`）；watchdog 在报告 ready 前固定父进程身份，POSIX 使用继承的 pidfd，Windows 使用 process handle 与 creation FILETIME，避免 PID 重用误杀。完成 event bus 连接与 durable consumer provisioning 后，才用 owner-aware compare-replace 将原 payload CAS 为 `READY`。peer 只接受精确 generation/role/instance、protocol v2、phase=`READY` 的合法 owner。claim/replace/poll/refresh 异常、peer 超时、畸形 payload、ownership 丢失、Redis 持续失败或缺 generation 均失败关闭。

consumer 在 `PROVISIONING` 时可注册，但 `NatsDeliveryGate` 会阻断 callback 的 parse/persist/handler/ack/nak；build 期 publish 只进入最多 4,096 条且 64 MiB 的双上限内存缓冲。仅 strict 四主进程 NATS/hybrid 路径注入 gate 并将 push consumer 的 `max_ack_pending` 固定为 `1`；non-strict、in-memory 与 monolith 不注入 gate，也不强制窗口为 `1`。checker/runtime 共用 mutable migration policy：只允许 snapshot/transient 正数旧 `ack_wait` 向声明目标提高；event `ack_wait`、任意 `max_deliver` 和不安全非事件 drift 阻断。携带关键 ALL cursor 的 consumer 遇到 safety-projection immutable drift 时失败关闭，不能删除重建。现存 durable 在 update/bind 前冻结 created、真实 inbox 与四维 cursor，回读/post-bind/READY/steady supervision 都拒绝同名重建、inbox 变化或 cursor 回退；实际 inbox 不进入日志/证据。flow control/idle heartbeat 历史差异当前仅保留并告警。本 role 升级 `READY` 且所有同 generation peer 均为 `READY` 后，先 flush 缓冲，再原子开放 callback，最后启动 background tasks；`ABORT` 是 sticky 且在竞态中优先。lease 使用 TTL 60 秒、每 10 秒续租、30 秒安全停机 margin 和 10 秒 hard-exit grace；每次成功 `PROVISIONING` 写入/续租只把本地 hard fence 滑动到该次写入后最多 50 秒，不能延长 claim 到 `READY` 的 180 秒绝对 promotion deadline；第 170 秒必须冻结续租并进入最后 10 秒 fatal grace。compare-replace/refresh 确定失租则零宽限硬退出。关键故障先冻结续租、ABORT gate 并锁定不可解除的 fatal deadline；正常停机也先冻结续租并把 cleanup 收紧到 10 秒，业务与 NATS 安全停止后才 disarm watchdog、owner-aware delete。其他路径保留 TTL fencing。

LAST/NEW durable 只表达“最新快照/订阅后新事件”，不承载 ALL 金融 cursor。其 ACK-window backlog 重建只在 strict delivery-gated、policy 非 ALL，并且（outstanding 超过目标，或窗口正在收缩且 outstanding 非零）时发生；safety-projection immutable drift 另有声明丢弃语义重建分支。两者都在 bind 前重建 consumer：LAST 重新取得当时最新快照，NEW 只接收重建后的新事件，旧 pending/cursor 按声明语义丢弃。标准 preflight 会先阻断 immutable drift，故该运行时分支不是发布绕行许可。自动重启也可能进入分支，代码不接收也不校验“已 full-down”信号。标准 full-down 是首次 v1 -> v2 切换的额外发布门禁。ALL durable 无论窗口或配置如何都禁止删除重建。

Compose Redis 使用 `noeviction`：容量不足必须显式写失败并由上层失败关闭，不能静默淘汰 owner key。NATS 允许底层无限重连，但断连连续 30 秒或连接永久关闭会形成 critical task failure，触发上述冻结与有界退出。standard deploy 在任何 mutation 前取得由单一长寿命 WSL `flock` holder 持有的全流程锁；生产锁路径固定为 `/tmp/aats-standard-deploy.lock`，`AATS_DEPLOY_LOCK_FILE` 仅在 `AATS_DEPLOY_TEST_MODE=true` 时可用于隔离测试，生产覆盖会在持锁前失败。Windows 每 3 秒刷新 lease，WSL holder 在 12 秒未刷新时释放 fd；新 holder 即使取得 flock，也必须等待其他仍新鲜（不超过 12 秒）的 predecessor lease 清除或过期后才返回 `ACQUIRED`，期间持续刷新自己的 lease。锁贯穿同步、构建、停启、迁移、健康检查、最终证据与报告，锁竞争或丢失均失败关闭。每个外部步骤 spawn 前先校验 holder/heartbeat/flock，并拒绝嵌套监督；spawn 后立即全局登记 active child PID/context，活动期间丢锁会终止并 wait 整个进程树。`TERM/HUP/INT/EXIT` 统一先终止并 wait active child，再停止并 wait heartbeat、移除本次 lease、关闭 holder fd 并 wait/终止 flock holder，不能先释放锁让前一子进程继续 mutation。cutover 先停止所有已知 profile 的七个应用容器，只接受 `exited/dead` 或明确 not-found，并记录容器 ID、状态、`StartedAt`、`FinishedAt`、`RestartCount`；基础设施-only up 后执行第一次 loopback 全量分页只读 preflight，前后重读上述指纹并查询该精确时间窗的 Docker lifecycle events。PASS 后 full-down 并建立新 quiescence 基线；正常 infra/schema 完成后、app up 前执行第二次同等 preflight。schema v3 使用人工 authoritative ownership manifest，并由动态 assembly 测试证明与四个 `build_runtime()` 精确一致：`77` 个 durable，角色 `31/8/27/11`、语义 `49/24/4`。preserved install 要求 exact `77` 且无 unowned；fresh install preflight 要求为空，app-up 后 final 才要求 exact `77`。它核验分页/总数、逐 stream count、identity/role/topic/semantics、created、四维 cursor、safety-projection config、窗口和 outstanding，不执行 ACK/update/delete/reset/purge。失败时保留 NATS 在线。v3 PASS 证据绑定 lock id、generation、部署提交与 quiescence，最终部署包重算 continuity、记录 path/hash，并封存两次 no-secret canonical durable projection/hash。monolith/in-memory 不启用跨进程 owner 协议。v3 已提交且标准第一次 preflight 已按设计阻断；两次 PASS chain、app-up、最终 evidence、Redis/NATS/Docker 完整故障注入、逐角色重启、双故障及下游 fencing 仍 `OPEN`，因此不等于完整业务健康或 trading-ready，真实资金继续 `NO-GO`。

运行事实边界（2026-08-29 05:54Z）：提交 `45612697` 的标准 derivatives 流程已运行 schema v3 `pre_full_down` 只读 preflight；它扫描 `3` 个 stream/`78` 个 consumer，识别全部 `77` 个声明 durable、无 missing，仅因 `aats-codex_manual_resume-system_operator_command_responses` 未归属而 `BLOCKED`。它没有进入 full-down/app-up；七个 app 保持停止，NATS/Redis/Postgres 健康在线。未知 durable 只能经真人 owner/release review 处置，不得自动删除。
