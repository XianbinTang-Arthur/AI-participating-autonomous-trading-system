...
consumer 在 `PROVISIONING` 时可注册，但 `NatsDeliveryGate` 会阻断 callback 的 parse/persist/handler/ack/nak；build 期 publish 只进入最多 4,096 条且 64 MiB 的双上限内存缓冲。仅 strict 四主进程 NATS/hybrid 路径注入 gate 并将 push consumer 的 `max_ack_pending` 固定为 `1`；non-strict、in-memory 与 monolith 不注入 gate，也不强制窗口为 `1`。已有 durable 的可变字段通过原位更新并回读验证，携带关键 ALL cursor 的 consumer 遇到不可变 drift 时失败关闭，不能删除重建。本 role 升级 `READY` 且所有同 generation peer 均为 `READY` 后，先 flush 缓冲，再原子开放 callback，最后启动 background tasks；`ABORT` 是 sticky 且在竞态中优先。lease 使用 TTL 60 秒、每 10 秒续租、30 秒安全停机 margin 和 10 秒 hard-exit grace；每次成功 `PROVISIONING` 写入/续租只把本地 hard fence 滑动到该次写入后最多 50 秒，不能延长 claim 到 `READY` 的 180 秒绝对 promotion deadline；第 170 秒必须冻结续租并进入最后 10 秒 fatal grace。compare-replace/refresh 确定失租则零宽限硬退出。关键故障先冻结续租、ABORT gate 并锁定不可解除的 fatal deadline；正常停机也先冻结续租并把 cleanup 收紧到 10 秒，业务与 NATS 安全停止后才 disarm watchdog、owner-aware delete。其他路径保留 TTL fencing。

LAST/NEW durable 只表达“最新快照/订阅后新事件”，不承载 ALL 金融 cursor。运行时同时满足 strict delivery-gated、policy 非 ALL，并且（outstanding 超过目标，或窗口正在收缩且 outstanding 非零）时，bind 可按声明语义重建 consumer：LAST 重新取得当时最新快照，NEW 只接收重建后的新事件，旧 pending/cursor 按声明语义丢弃；自动重启也可能进入该分支，代码不接收也不校验“已 full-down”信号。标准 full-down 是首次 v1 -> v2 切换的额外发布门禁，用于取得停产、quiescence 与只读 preflight 证据，不能冒充运行时边界。ALL durable 无论窗口或配置如何都禁止删除重建。

Compose Redis 使用 `noeviction`：容量不足必须显式写失败并由上层失败关闭，不能静默淘汰 owner key。NATS 允许底层无限重连，但断连连续 30 秒或连接永久关闭会形成 critical task failure，触发上述冻结与有界退出。standard deploy 在任何 mutation 前取得由单一长寿命 WSL `flock` holder 持有的全流程锁；生产锁路径固定为 `/tmp/aats-standard-deploy.lock`，`AATS_DEPLOY_LOCK_FILE` 仅在 `AATS_DEPLOY_TEST_MODE=true` 时可用于隔离测试，生产覆盖会在持锁前失败。Windows 每 3 秒刷新 lease，WSL holder 在 12 秒未刷新时释放 fd；新 holder 即使取得 flock，也必须等待其他仍新鲜（不超过 12 秒）的 predecessor lease 清除或过期后才返回 `ACQUIRED`，期间持续刷新自己的 lease。锁贯穿同步、构建、停启、迁移、健康检查、最终证据与报告，锁竞争或丢失均失败关闭。每个外部步骤 spawn 前先校验 holder/heartbeat/flock，并拒绝嵌套监督；spawn 后立即全局登记 active child PID/context，活动期间丢锁会终止并 wait 整个进程树。`TERM/HUP/INT/EXIT` 统一先终止并 wait active child，再停止并 wait heartbeat、移除本次 lease、关闭 holder fd 并 wait/终止 flock holder，不能先释放锁让前一子进程继续 mutation。cutover 先停止所有已知 profile 的七个应用容器，只接受 `exited/dead` 或明确 not-found，并记录容器 ID、状态、`StartedAt`、`FinishedAt`、`RestartCount`；基础设施-only up 后执行第一次 loopback 全量分页只读 preflight，前后重读上述指纹并查询该精确时间窗的 Docker lifecycle events。PASS 后 full-down 并建立新 quiescence 基线；正常 infra/schema 完成后、app up 前执行第二次同等 preflight。它核验目标 stream/filter/ack/deliver policy、created/cursor、窗口和 outstanding，不执行 ACK/update/delete/reset/purge。失败时保留 NATS 在线。v2 PASS 证据绑定 lock id、generation、部署提交与 quiescence，最终部署包校验这些字段一致并记录相对路径与 SHA-256。monolith/in-memory 不启用跨进程 owner 协议。针对新部署互斥的运行证据目前仅有 Windows Git Bash -> WSL 聚焦 smoke，不是完整 standard deploy；真实 standard deploy、Redis/NATS/Docker 完整故障注入、逐角色重启、双故障及下游 fencing 仍 `OPEN`，因此不等于完整业务健康或 trading-ready，真实资金继续 `NO-GO`。
