...
class TestDashboardUI(unittest.TestCase):
    def test_dashboard_routes_serve_html_and_assets_when_auth_is_disabled(self) -> None:
        ...
        self.assertEqual(admin_js.status_code, 200)
        self.assertEqual(terms_js.status_code, 200)
        ...
        self.assertIn("策略档位、账号与权限", settings.text)

        self.assertIn("window.location.replace(\"/login\")", js.text)
        self.assertIn("evaluate-strategy-profile", js.text)
        self.assertIn("accept-strategy-profile-now", js.text)
        self.assertIn("stage-strategy-profile", js.text)
        self.assertIn("activate-pending-strategy-profile", js.text)
        self.assertIn("rollback-strategy-profile", js.text)
        ...
        self.assertIn("策略档位状态", admin_js.text)
