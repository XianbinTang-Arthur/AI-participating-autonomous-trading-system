...
def test_ioc_quantity_is_capped_by_ohlcv_participation() -> None:
    result = FillSimulator(max_volume_participation=Decimal("0.01")).simulate(
    ...
    assert result.avg_fill_price == Decimal("100.0100")
...
def test_bounded_limit_uses_capped_taker_fallback() -> None:
    ...
    assert result.filled_qty == Decimal("1.00")
    assert result.avg_fill_price == Decimal("99.9900")
    assert result.fee_bps == 5.0
    assert result.slippage_bps == 1.0
...
@pytest.mark.parametrize(
    ...
    [
        (
            BacktestConfig(fill_model_version="legacy_full_fill_v1"),  # type: ignore[arg-type]
            "Unsupported fill_model_version",
        ),
        (
            BacktestConfig(max_volume_participation=Decimal("0")),
            "max_volume_participation",
        ),
        (
            BacktestConfig(max_volume_participation=Decimal("1.01")),
            "max_volume_participation",
        ),
    ],
)
...
def test_cost_diagnostic_preserves_actual_fee_and_slippage_components() -> None:
    ...
    assert FILL_MODEL_VERSION == "ohlcv_participation_cap_v2"
