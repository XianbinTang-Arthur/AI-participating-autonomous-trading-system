...
def test_release_failure_does_not_skip_existing_rollback_enforcement() -> None:
    history = {
        "releases": [
            {
                "release_id": "rel_malformed",
                "family": "independent",
                "timeframe": "15m",
                "apply_result": "success",
                "observation_status": "observing",
    ...
    assert result["processed_count"] == 1
    assert result["auto_rollback_count"] == 1
    assert result["results"][0]["error"] == "release_observation_stage_failed"
    assert result["results"][0]["stage_errors"][0]["stage"] == "observation"
    rollback_mock.assert_called_once()
    effectiveness_mock.assert_called_once()
    enforce_mock.assert_called_once()


def test_rollback_stage_failure_does_not_skip_risk_materialization() -> None:
    history = {
        "releases": [
            {
                "release_id": "rel_observation_risk",
                "family": "independent",
                "timeframe": "15m",
                "apply_result": "success",
                "observation_status": "observing",
    ...
    with (
        ...
        patch(
            "aats.data_platform.production_workflow.rollback_policy."
            "evaluate_rollback_recommendation",
            side_effect=RuntimeError("sibling evaluator failed"),
        ),
        patch(
            "aats.data_platform.metrics.release_effectiveness."
            "evaluate_release_effectiveness",
            return_value={"conclusion": "rollback_triggered"},
        ) as effectiveness_mock,
        patch(
            "aats.data_platform.metrics.release_effectiveness."
            "enforce_pending_rollbacks",
            return_value=[{"release_id": "rel_observation_risk", "ok": True}],
        ) as enforce_mock,
    ):
        result = run_release_observation_cycle(Path("."), save_results=True)
    ...
    assert result["ok"] is False
    assert result["auto_rollback_count"] == 1
    assert result["results"][0]["stage_errors"] == [
        {
