...
class TestRunBacktestTrades(unittest.TestCase):
    ...
    def test_run_backtest_single_bar_open_long(self) -> None:
        """delta>0 + ioc → 1 taker fill，accumulated_fees > 0。"""
        ...
        self.assertEqual(result.summary.fill_count, 1)
        self.assertGreater(result.summary.fee_total, Decimal("0"))
        # cost validator 应记录 1 条
    ...
    def test_run_backtest_sequence_entry_exit(self) -> None:
        ...
        self.assertEqual(result.summary.bar_count, 3)
        # PnL = 1 * (51000 - 50005) * 0.01 - 2*slip ≈ positive
...
class TestRunBacktestOrderType(unittest.TestCase):
    def test_run_backtest_respects_order_type_post_only(self) -> None:
        ...
        if result.fills_count == 1:
            # maker fee = 2 bps, 5 * 50000 * 2/10000 = 50
            self.assertAlmostEqual(
                float(result.summary.fee_total),
                float(Decimal("5") * Decimal("50000") * Decimal("0.0002")),
                places=6,
            )
