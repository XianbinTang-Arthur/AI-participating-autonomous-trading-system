...
> 审计日期：2026-08-22 至 2026-08-23  
> 代码基线：`be9179ead5be6aba22fbe94e3baf72b9f46eedc3`（`main`）  
> 审计对象：基线内 699 个 Git 跟踪 Markdown 文档、`docs/task/` 下两份 legacy Word 文档，以及与文档语义直接相关的代码注释和基础设施注释  
...
| 本地 paper loop | `scripts/run_local.py` 可直接运行 | 该脚本仍调用旧 decision 入口签名，当前不可作为受支持入口 |
...
| 部署健康门 | derivatives-live 的 7 个应用进程都被部署脚本强制检查 | 部署脚本当前 required health 只覆盖主 5 个；两个采集器仍需单独核验 |
...
1. `scripts/run_local.py` 与当前 decision 入口不兼容；
2. `deploy.sh` 的 required health list 未覆盖 derivatives-live 两个采集器；
