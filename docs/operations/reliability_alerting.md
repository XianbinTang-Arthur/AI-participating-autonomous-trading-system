...
| `workflow_configs_exist` | operations | critical | 4 个 workflow 配置文件都存在 |
...
| `active_parameters` | decision | info | active_parameters_registry.json 可读 |
...
在 `decision_cycle` workflow 中已包含可靠性检查任务：
...
可通过解析退出码实现与外部监控系统的集成：

```bash
python scripts/rdp_run_reliability_check.py
exit_code=$?
if [ $exit_code -eq 1 ]; then
    # 发送 critical 告警到 Slack / PagerDuty / etc.
    curl -X POST "https://hooks.slack.com/..." -d '{"text":"RDP Critical Alert!"}'
fi
```
