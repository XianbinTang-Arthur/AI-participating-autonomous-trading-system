-- Migration 0007: Create silver candle tables
...
BEGIN
    FOREACH tbl IN ARRAY tbls LOOP
        ...
        EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %s (ts)',
            'idx_' || replace(replace(tbl, '.', '_'), 'silver_', 'slv_') || '_ts', tbl);
        EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %s (ingest_run_id)',
            'idx_' || replace(replace(tbl, '.', '_'), 'silver_', 'slv_') || '_run', tbl);
        EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %s (dataset_version)',
            'idx_' || replace(replace(tbl, '.', '_'), 'silver_', 'slv_') || '_ver', tbl);
    END LOOP;
END
$body$;
