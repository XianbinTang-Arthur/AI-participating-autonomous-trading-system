...
标准部署在任何 mutation 前取得并持续持有长寿命 WSL `flock`。生产锁路径固定为 `/tmp/aats-standard-deploy.lock`；只有 `AATS_DEPLOY_TEST_MODE=true` 的隔离测试才允许通过 `AATS_DEPLOY_LOCK_FILE` 改路径，生产覆盖直接失败。Windows 端每 3 秒更新 lease，WSL holder 在 12 秒失联后释放锁；新 holder 取得 flock 后若发现其他仍新鲜的 predecessor lease，会由 holder 持续刷新自己的 lease 并隔离等待，直至前任 lease 清除或过期才报告取得锁。每个外部步骤 spawn 前复核 holder/heartbeat/flock，spawn 后立即全局登记 active child PID/context；活动中丢锁会终止并 wait 进程树。`TERM/HUP/INT/EXIT` 必须先终止并 wait active child，再按 heartbeat、lease、holder fd/flock 的顺序释放，禁止先放锁后遗留 mutation。cutover 会停止所有已知 profile 的七个应用容器，只接受 `exited/dead` 或明确 not-found，记录并前后比较容器 ID、状态、`StartedAt`、`FinishedAt`、`RestartCount`，同时查询 preflight 精确时间窗内的 Docker lifecycle events。第一次只读 preflight 位于基础设施-only up 后、full-down 前；full-down 后重建 quiescence 基线，正常 infra/schema 完成后、app up 前再执行第二次。schema v3 使用人工 authoritative ownership manifest，并由动态 assembly 测试证明与四个 `build_runtime()` 精确一致：`77` 个 durable，角色 `31/8/27/11`，语义 `49/24/4`。preserved install 要求 exact `77` 且无 unowned；fresh install preflight 要求为空，app-up 后 final 才要求 exact `77`。任何 outstanding/config drift/查询失败或 quiescence 变化都必须保留 NATS 在线并阻断。两次 quiescent 快照仅允许 `LIMITS + discard=old + max_age>0` stream 的连续头部到龄淘汰：created/config/last sequence/consumer count 必须不变，messages 与 bytes 严格下降，first sequence 前移量必须等于消息减少量，deleted/num_deleted 必须为空/零，且全部 durable 投影逐行相等；证据必须记录差量与 `purge_vs_expiry_not_distinguishable` 信任边界，其他变化仍失败关闭。v3 PASS 绑定 lock id、generation、部署提交与 quiescence，最终证据重算 continuity、记录 path/hash，并封存两次 no-secret canonical durable projection/hash。protocol v1 -> v2 首次发布和回滚禁止 rolling/mixed version；标准 stop 本身不是 drain 证明，BLOCKED outstanding 只能在人工批准变更窗用匹配旧消费者自然 drain 后重跑，禁止自动 ACK/delete/update/recreate/reset/purge。LAST/NEW 的 ACK-window backlog 与 immutable-drift 重建是两个声明丢弃语义分支；标准 preflight 会先阻断 immutable drift，full-down 是额外发布门禁，不是运行时信号。当前 v3 代码已提交并通过静态/单元审查；两次 PASS chain、app-up 与最终 evidence 必须以最新标准部署运行证据为准，完整故障矩阵保持 `OPEN`。

WSL completion wrapper 在同步子命令等待期收到 HUP/INT/TERM 时必须延迟处理：先等待子命令返回、计算输出摘要并原子发布 ACK，再传播 transport 非零状态。guard 对 transport 非零也先验证 ACK；只有有效 ACK 与捕获字节完全绑定时才可清 marker，但本次部署仍返回 16。缺失、畸形或不匹配 ACK 继续保留 marker；SIGKILL 等无 handler 情形仍要求人工恢复。encoded wrapper 先由静态 WSL loader 完整读取，再以关闭的 stdin 启动真实 wrapper；禁止把 ACK 程序直接作为远端 Bash stdin，以免 Docker/Compose 吞掉未解析尾部。真实 wsl.exe 调用禁用 MSYS argv 路径改写。wrapper 内部失败只允许输出 allowlisted phase 与数值状态，guard 只补充 transport 状态；不得记录远端命令、捕获输出或凭证，也不得把诊断 side effect 当完成证明。

当前运行边界（2026-08-29 EDT）：真人明确授权后，唯一未归属且 `pending/ack pending/redelivered` 均为零的 `aats-codex_manual_resume-system_operator_command_responses` 已精确删除；声明的 `77` 个 durable 未删除、未 ACK、未 reset/purge/recreate。独立 Windows 计划任务重现证明 schema 后 ACK 缺失不是 Codex 前台超时：诊断固定为 `hash_output/status=0`，隔离探针进一步证明远端命令读取 stdin 会吞掉 ACK 程序尾部；修复后真实标准入口已越过 schema/ACK。随后第二次 preflight 只因两个 24 小时 `LIMITS` stream 在 124.620 秒窗口内出现连续头部到龄淘汰而失败，77 个 durable 逐行相等、last sequence 均未增长；该误判的最小 fail-closed 修复已通过相关 139 项和全量 `6565 passed, 32 skipped, 259 subtests passed`，标准部署仍须重跑形成最终 evidence。当前七个 app 保持停止，九个基础设施容器健康在线；不得手工启动 app 绕过门禁。
