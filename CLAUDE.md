...
WSL completion wrapper 在同步子命令等待期收到 HUP/INT/TERM 时必须延迟处理：先等待子命令返回、计算输出摘要并原子发布 ACK，再传播 transport 非零状态。guard 对 transport 非零也先验证 ACK；只有有效 ACK 与捕获字节完全绑定时才可清 marker，但本次部署仍返回 16。缺失、畸形或不匹配 ACK 继续保留 marker；SIGKILL 等无 handler 情形仍要求人工恢复。
