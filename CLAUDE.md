...
当前运行边界（2026-08-29 EDT）：真人明确授权后，唯一未归属且 `pending/ack pending/redelivered` 均为零的 `aats-codex_manual_resume-system_operator_command_responses` 已精确删除；声明的 `77` 个 durable 未删除、未 ACK、未 reset/purge/recreate。独立 Windows 计划任务重现证明 schema 后 ACK 缺失不是 Codex 前台超时：诊断固定为 `hash_output/status=0`，隔离探针进一步证明远端命令读取 stdin 会吞掉 ACK 程序尾部。transport 修复的 87 项定向单元、全量 `6555 passed, 32 skipped` 与 Windows Git Bash → WSL 锁 smoke 已通过；标准部署重跑尚未验收。当前七个 app 保持停止，九个基础设施容器健康在线；poison marker 仅可在复核两库账本、无迁移进程/锁且 lease stale 后按精确路径人工恢复，不得手工启动 app 绕过门禁。
