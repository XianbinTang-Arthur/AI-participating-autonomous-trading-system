...
> 文档状态：现行约束。最后核对：2026-08-29（起始 HEAD `ae8ba038e249ce6a4df3303691c2d32e46fd9c6d`；包含当前 FS-016 readiness lease 与 WSL completion ACK 信号安全候选，以本文档所在最终 HEAD 为准）。代理权限、实施、验证或部署纪律变化时必须同步复核。
...
- **WSL completion ACK**: HUP/INT/TERM 必须等待同步子命令返回并发布 ACK；transport 非零时，有效且与捕获字节绑定的 ACK 只允许清除 mutation 完成歧义，部署仍失败。缺失/坏 ACK 与 SIGKILL 情形保留 marker，禁止自动接管
