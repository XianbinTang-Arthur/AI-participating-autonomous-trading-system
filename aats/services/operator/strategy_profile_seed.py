...
def _balanced_profile_payload(payload: StrategyProfilePayload) -> StrategyProfilePayload:
    return _copy_payload(
        ...
        strategy_min_net_edge_bps=_clamp_float(payload.strategy_min_net_edge_bps, lower=3.0, upper=4.0),
        strategy_entry_alpha_min=_clamp_float(payload.strategy_entry_alpha_min, lower=0.16, upper=0.18),
        strategy_entry_confidence_min=_clamp_float(payload.strategy_entry_confidence_min, lower=0.62, upper=0.66),
        strategy_scale_in_alpha_min=_clamp_float(payload.strategy_scale_in_alpha_min, lower=0.20, upper=0.24),
        strategy_scale_in_confidence_min=_clamp_float(payload.strategy_scale_in_confidence_min, lower=0.66, upper=0.70),
        strategy_reversal_alpha_min=_clamp_float(payload.strategy_reversal_alpha_min, lower=0.30, upper=0.34),
        strategy_reversal_confidence_min=_clamp_float(payload.strategy_reversal_confidence_min, lower=0.70, upper=0.78),
        strategy_min_hold_seconds=_clamp_float(payload.strategy_min_hold_seconds, lower=480.0, upper=720.0),
        strategy_post_close_cooldown_seconds=_clamp_float(
            payload.strategy_post_close_cooldown_seconds,
            lower=180.0,
            upper=300.0,
        ),
        strategy_low_edge_threshold_bps=_clamp_float(payload.strategy_low_edge_threshold_bps, lower=3.0, upper=4.0),
        strategy_low_edge_streak_limit=max(int(payload.strategy_low_edge_streak_limit), 4),
    ...
    return [
        StrategyProfileRevision(
            ...
            payload=_copy_payload(
                ...
                decision_min_price_move_bps=min(balanced_payload.decision_min_price_move_bps, 3.0),
                decision_min_momentum_delta=min(balanced_payload.decision_min_momentum_delta, 0.00025),
                strategy_expected_slippage_bps_fraction=min(
                    balanced_payload.strategy_expected_slippage_bps_fraction,
                    0.25,
                ),
                strategy_min_net_edge_bps=min(balanced_payload.strategy_min_net_edge_bps, 2.5),
                strategy_entry_alpha_min=min(balanced_payload.strategy_entry_alpha_min, 0.14),
                strategy_entry_confidence_min=min(balanced_payload.strategy_entry_confidence_min, 0.60),
                strategy_scale_in_alpha_min=min(balanced_payload.strategy_scale_in_alpha_min, 0.18),
                strategy_scale_in_confidence_min=min(balanced_payload.strategy_scale_in_confidence_min, 0.64),
                strategy_reversal_alpha_min=max(balanced_payload.strategy_reversal_alpha_min, 0.32),
                strategy_reversal_confidence_min=max(balanced_payload.strategy_reversal_confidence_min, 0.74),
                strategy_min_hold_seconds=min(balanced_payload.strategy_min_hold_seconds, 360.0),
                strategy_post_close_cooldown_seconds=min(
                    balanced_payload.strategy_post_close_cooldown_seconds,
                    120.0,
                ),
                strategy_low_edge_threshold_bps=min(balanced_payload.strategy_low_edge_threshold_bps, 3.0),
        ...
        StrategyProfileRevision(
            ...
            payload=_copy_payload(
                ...
                decision_min_price_move_bps=max(balanced_payload.decision_min_price_move_bps, 4.5),
                decision_min_momentum_delta=max(balanced_payload.decision_min_momentum_delta, 0.0004),
                strategy_expected_slippage_bps_fraction=max(
                    balanced_payload.strategy_expected_slippage_bps_fraction,
                    0.28,
                ),
                strategy_min_net_edge_bps=max(balanced_payload.strategy_min_net_edge_bps, 5.0),
                strategy_entry_alpha_min=max(balanced_payload.strategy_entry_alpha_min, 0.19),
                strategy_entry_confidence_min=max(balanced_payload.strategy_entry_confidence_min, 0.68),
                strategy_scale_in_alpha_min=max(balanced_payload.strategy_scale_in_alpha_min, 0.24),
                strategy_scale_in_confidence_min=max(balanced_payload.strategy_scale_in_confidence_min, 0.74),
                strategy_reversal_alpha_min=max(balanced_payload.strategy_reversal_alpha_min, 0.36),
                strategy_reversal_confidence_min=max(balanced_payload.strategy_reversal_confidence_min, 0.80),
                strategy_min_hold_seconds=max(balanced_payload.strategy_min_hold_seconds, 840.0),
                strategy_post_close_cooldown_seconds=max(
                    balanced_payload.strategy_post_close_cooldown_seconds,
                    420.0,
                ),
                strategy_low_edge_threshold_bps=max(balanced_payload.strategy_low_edge_threshold_bps, 4.0),
                strategy_low_edge_streak_limit=min(balanced_payload.strategy_low_edge_streak_limit, 4),
                strategy_low_edge_cooldown_seconds=max(balanced_payload.strategy_low_edge_cooldown_seconds, 2_400.0),
        ...
        StrategyProfileRevision(
            ...
            payload=_copy_payload(
                balanced_payload,
                decision_min_interval_seconds_15m=max(balanced_payload.decision_min_interval_seconds_15m, 90.0),
                max_decisions_per_minute=min(balanced_payload.max_decisions_per_minute, 2),
                decision_min_price_move_bps=max(balanced_payload.decision_min_price_move_bps, 5.5),
                decision_min_momentum_delta=max(balanced_payload.decision_min_momentum_delta, 0.0006),
                strategy_expected_slippage_bps_fraction=max(
                    balanced_payload.strategy_expected_slippage_bps_fraction,
                    0.30,
                ),
                strategy_min_net_edge_bps=max(balanced_payload.strategy_min_net_edge_bps, 7.0),
                strategy_entry_allowed_regimes=("breakout",),
                strategy_entry_alpha_min=max(balanced_payload.strategy_entry_alpha_min, 0.22),
                strategy_entry_confidence_min=max(balanced_payload.strategy_entry_confidence_min, 0.70),
                strategy_scale_in_alpha_min=max(balanced_payload.strategy_scale_in_alpha_min, 0.28),
                strategy_scale_in_confidence_min=max(balanced_payload.strategy_scale_in_confidence_min, 0.78),
                strategy_reversal_alpha_min=max(balanced_payload.strategy_reversal_alpha_min, 0.40),
                strategy_reversal_confidence_min=max(balanced_payload.strategy_reversal_confidence_min, 0.84),
                strategy_min_hold_seconds=max(balanced_payload.strategy_min_hold_seconds, 1_200.0),
                strategy_post_close_cooldown_seconds=max(
                    balanced_payload.strategy_post_close_cooldown_seconds,
                    900.0,
                ),
                strategy_low_edge_threshold_bps=max(balanced_payload.strategy_low_edge_threshold_bps, 4.5),
                strategy_low_edge_streak_limit=min(balanced_payload.strategy_low_edge_streak_limit, 3),
                strategy_low_edge_cooldown_seconds=max(balanced_payload.strategy_low_edge_cooldown_seconds, 3_600.0),
                strategy_transient_close_retry_cooldown_seconds=max(
                    balanced_payload.strategy_transient_close_retry_cooldown_seconds,
                    180.0,
                ),
        ...
        StrategyProfileRevision(
            ...
            payload=_copy_payload(
                ...
                decision_min_price_move_bps=max(balanced_payload.decision_min_price_move_bps, 7.5),
                decision_min_momentum_delta=max(balanced_payload.decision_min_momentum_delta, 0.0008),
                strategy_expected_slippage_bps_fraction=max(
                    balanced_payload.strategy_expected_slippage_bps_fraction,
                    0.32,
                ),
                strategy_min_net_edge_bps=max(balanced_payload.strategy_min_net_edge_bps, 9.0),
                strategy_entry_allowed_regimes=("breakout",),
                strategy_entry_alpha_min=max(balanced_payload.strategy_entry_alpha_min, 0.26),
                strategy_entry_confidence_min=max(balanced_payload.strategy_entry_confidence_min, 0.76),
                strategy_scale_in_alpha_min=max(balanced_payload.strategy_scale_in_alpha_min, 0.32),
                strategy_scale_in_confidence_min=max(balanced_payload.strategy_scale_in_confidence_min, 0.82),
                ...
                strategy_low_edge_threshold_bps=max(balanced_payload.strategy_low_edge_threshold_bps, 4.5),
                strategy_low_edge_streak_limit=min(balanced_payload.strategy_low_edge_streak_limit, 3),
                strategy_low_edge_cooldown_seconds=max(balanced_payload.strategy_low_edge_cooldown_seconds, 4_800.0),
                strategy_transient_close_retry_cooldown_seconds=max(
                    balanced_payload.strategy_transient_close_retry_cooldown_seconds,
                    240.0,
                ),
        ...
        StrategyProfileRevision(
            ...
            payload=_copy_payload(
                ...
                decision_min_price_move_bps=max(balanced_payload.decision_min_price_move_bps, 9.0),
                decision_min_momentum_delta=max(balanced_payload.decision_min_momentum_delta, 0.001),
                strategy_expected_slippage_bps_fraction=max(
                    balanced_payload.strategy_expected_slippage_bps_fraction,
                    0.35,
                ),
                strategy_min_net_edge_bps=max(balanced_payload.strategy_min_net_edge_bps, 11.0),
                strategy_entry_allowed_regimes=("breakout",),
                strategy_entry_alpha_min=max(balanced_payload.strategy_entry_alpha_min, 0.30),
                strategy_entry_confidence_min=max(balanced_payload.strategy_entry_confidence_min, 0.80),
                strategy_scale_in_alpha_min=max(balanced_payload.strategy_scale_in_alpha_min, 0.36),
                strategy_scale_in_confidence_min=max(balanced_payload.strategy_scale_in_confidence_min, 0.86),
