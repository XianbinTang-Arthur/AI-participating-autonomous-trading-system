    ...
    *,
    ...
    session.execute(
        text("""
            INSERT INTO governance.recommendations
                (recommendation_id, family, symbol, timeframe,
                 ...
                 superseded_at, superseded_by_recommendation_id,
                 created_at)
            VALUES
                (:rec_id, :family, :symbol, :timeframe,
                 ...
                 :status, :approved_by, :approved_at, :approval_notes,
                 :rejected_by, :rejected_at,
                 :superseded_at, :superseded_by_rec_id,
                 :created_at)
        ...
        """),
        {
            ...
            "rejected_at": _parse_dt(rejected_at),
...
def db_update_recommendation_status(
    ...
    *,
    ...
    if approved_at is not None:
        ...
        params["approved_at"] = _parse_dt(approved_at) or datetime.now(timezone.utc)
    if approval_notes is not None:
        set_parts.append("approval_notes = :approval_notes")
        params["approval_notes"] = approval_notes
    if rejected_by is not None:
        set_parts.append("rejected_by = :rejected_by")
        params["rejected_by"] = rejected_by
    if rejected_at is not None:
        set_parts.append("rejected_at = :rejected_at")
        params["rejected_at"] = _parse_dt(rejected_at) or datetime.now(timezone.utc)
    if superseded_at is not None:
        set_parts.append("superseded_at = :superseded_at")
        params["superseded_at"] = _parse_dt(superseded_at) or datetime.now(timezone.utc)
...
def _rec_row_to_dict(row: Any) -> dict[str, Any]:
    ...
    if row.approval_notes:
        d["approval_notes"] = row.approval_notes
    if row.rejected_by:
