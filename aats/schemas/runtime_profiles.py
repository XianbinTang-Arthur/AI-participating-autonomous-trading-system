...
RUNTIME_PROFILE_BASE_FIELDS: tuple[str, ...] = (
    "default_symbol",
    "allowed_symbols",
...
def runtime_profile_payload_from_settings(settings: AATSSettings) -> dict[str, Any]:
    ...
    fields = list(RUNTIME_PROFILE_BASE_FIELDS)
    if settings.trading_product_type == "derivatives":
        fields.extend(RUNTIME_PROFILE_DERIVATIVES_ONLY_FIELDS)
...
def summarize_runtime_profile_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("trading_product_type") == "derivatives":
        summary.update(
            {
