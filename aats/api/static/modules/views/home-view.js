    ...
    <div class="panel-grid">
      <div class="span-4">
        ${surfaceCard({
          ...
          content: alertQueue(primaryIssues({ blockers, reconciliation, recovery, account }), "当前没有需要立刻人工处理的问题。"),
        })}
      </div>
      ...
      <div class="span-4">
        ${surfaceCard({
          ...
          content: summaryStrip([
            ...
            { label: "控制权限", value: uiHints.controlPermissionMessage ? "受限" : "可写", meta: uiHints.controlPermissionMessage || "当前账号可以执行人工控制", tone: uiHints.controlPermissionMessage ? "warning" : "positive" },
          ...
          content: `
            <div class="callout">
              <div class="panel-head">
                <h3>${latestDecision.decision_id ? "最新动作已生成" : "当前没有新的交易动作"}</h3>
      ...
      <div class="span-6">
        ${surfaceCard({
          ...
          content: alertQueue(secondaryAlerts({ recovery, blockers, reconciliation, account, uiHints }), "当前没有新的次级提醒。"),
        })}
      </div>
    </div>
  `;
...
function displayExecutionRouteMeta(value) {
  const route = String(value || "").toLowerCase();
  if (!route || route === "unknown") return "当前还没有线路说明";
...
function latestActionNarrative({ latestDecision, latestOrder, latestFill, reconciliation }) {
  ...
  return `${formatRelativeAge(latestDecision.decision_time || latestDecision.decision_context?.as_of_ts)}，系统针对 ${symbol} 形成了 ${decisionText} 的判断。`
    ...
    + `${latestFill ? ` 最新一笔成交已落库，数量 ${formatNumber(latestFill.fill_qty)}。` : " 当前还没有新的成交落库。"}`
    + `${reconciliation ? ` 最近对账结论为 ${readableState(reconciliation.severity)}。` : " 最近还没有新的对账结论。"}`;
}
...
function primaryIssues({ blockers, reconciliation, recovery, account }) {
  ...
  if (reconciliation?.halt_required) {
    return [{
      title: "优先确认对账差异",
      copy: `最新对账结果为 ${readableState(reconciliation.severity)}，建议先完成状态核对，再考虑恢复自动交易。`,
  ...
  if (!recovery.safe_to_trade) {
    return [{
      title: "恢复条件尚未满足",
      copy: listOrDash(recovery.resume_blocked_reasons, "当前没有给出额外恢复限制说明"),
...
function secondaryAlerts({ recovery, blockers, reconciliation, account, uiHints }) {
  ...
  if (recovery.review_required) {
    items.push({
      title: "系统仍在等待人工复核",
      copy: uiHints.recoveryReasonsText || listOrDash(recovery.resume_blocked_reasons, "当前没有额外复核说明"),
      meta: readableState(recovery.recovery_state),
      tone: "warning",
      pill: pill("待复核", "warning"),
    });
  }
  if (blockers.length === 0 && reconciliation?.severity) {
    items.push({
      ...
      tone: reconciliation.halt_required ? "danger" : "info",
