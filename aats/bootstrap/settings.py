...
class AATSSettings(BaseSettings):
    ...
    runtime_readiness_generation: str | None = Field(
        default=None,
        description=(
            "Deployment-scoped generation for the Redis peer-readiness barrier. "
            "Required by split-process NATS/hybrid entrypoints; generated by deploy.sh."
        ),
    )
    ...
    # Round 3 · 2026-04-22 · Non-AI paper trading shadow (sibling of ai_shadow_*).
    # Runs candidate strategy params in parallel with live, records divergence
    # without executing. See docs/task/round3_paper_trading_design.md.
    # 默认 OFF —— 只有显式开启 + candidate 非空才会触发 shadow code path。
    paper_trading_shadow_enabled: bool = False
    # Each dict: {"candidate_id": str, "family": StrategyFamily, "overrides": dict}
    # 示例：
    #   ({"candidate_id": "independent_low_threshold", "family": "independent",
    #     "overrides": {"strategy_hedge_independent_long_entry_threshold": 0.15}},)
    paper_trading_shadow_candidates: tuple[dict, ...] = Field(default_factory=tuple)
    ...
    operator_login_max_concurrency: int = 4
    operator_login_queue_timeout_seconds: float = 1.0
    operator_login_rate_limit_window_seconds: float = 60.0
    operator_login_rate_limit_global_attempts: int = 60
    operator_login_rate_limit_client_attempts: int = 20
    operator_login_rate_limit_identity_attempts: int = 10
    ...
    @field_validator("runtime_readiness_generation", mode="before")
    @classmethod
    def normalize_runtime_readiness_generation(cls, value: Any) -> str | None:
        if value is None:
            return None
        if not isinstance(value, str):
            raise ValueError("runtime_readiness_generation_must_be_string_or_none")
        normalized = value.strip()
        if not normalized:
            return None
        if len(normalized) > 128 or re.fullmatch(r"[A-Za-z0-9._:-]+", normalized) is None:
            raise ValueError("runtime_readiness_generation_has_invalid_format")
        return normalized
    ...
    def validate_supported_runtime_overrides(self) -> "AATSSettings":
        ...
        if not 1 <= int(self.operator_login_max_concurrency) <= 32:
            raise ValueError("operator_login_max_concurrency_must_be_between_1_and_32")
        login_queue_timeout = float(self.operator_login_queue_timeout_seconds)
        if not math.isfinite(login_queue_timeout) or login_queue_timeout <= 0.0:
            raise ValueError(
                "operator_login_queue_timeout_seconds_must_be_finite_and_positive"
            )
        login_rate_limit_window = float(
            self.operator_login_rate_limit_window_seconds
        )
        if (
            not math.isfinite(login_rate_limit_window)
            or login_rate_limit_window <= 0.0
        ):
            raise ValueError(
                "operator_login_rate_limit_window_seconds_must_be_finite_and_positive"
            )
        login_global_limit = int(self.operator_login_rate_limit_global_attempts)
        login_client_limit = int(self.operator_login_rate_limit_client_attempts)
        login_identity_limit = int(self.operator_login_rate_limit_identity_attempts)
        if login_global_limit < 1:
            raise ValueError("operator_login_rate_limit_global_attempts_must_be_positive")
        if not 1 <= login_client_limit <= login_global_limit:
            raise ValueError(
                "operator_login_rate_limit_client_attempts_must_be_between_1_and_global_limit"
            )
        if not 1 <= login_identity_limit <= login_global_limit:
            raise ValueError(
                "operator_login_rate_limit_identity_attempts_must_be_between_1_and_global_limit"
            )
